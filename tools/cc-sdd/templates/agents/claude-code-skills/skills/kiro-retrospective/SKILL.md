...
- Identify the repo, the spec(s)/feature under `{{KIRO_DIR}}/specs/`, the skills used **this session** (you
...
- Create `{{KIRO_DIR}}/feedback/` if needed.
- Write `{{KIRO_DIR}}/feedback/feedback-<timestamp>[-<feature>].md` containing **only** the frontmatter stub
...
- Create `{{KIRO_DIR}}/retrospective/` if needed.
- Write `{{KIRO_DIR}}/retrospective/skill-improvements-<timestamp>[-<feature>].md` with the
