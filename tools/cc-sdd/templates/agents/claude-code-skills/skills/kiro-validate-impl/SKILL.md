...
- Read `{{KIRO_DIR}}/specs/<feature>/spec.json` for metadata (including `spec_type`, `workflow`, `artifacts`)
- Read `{{KIRO_DIR}}/specs/<feature>/requirements.md` for requirements
- **Design file loading based on lifecycle config**:
  - If `artifacts.design_hld` is enabled: read `{{KIRO_DIR}}/specs/<feature>/design-hld.md`
  - If `artifacts.design_lld` is enabled: read `{{KIRO_DIR}}/specs/<feature>/design-lld.md`
  - If neither HLD/LLD fields exist in artifacts (legacy spec): read `{{KIRO_DIR}}/specs/<feature>/design.md`
