...
metadata:
  shared-rules: "steering-principles.md, global-context-loading.md"
  shared-templates: "steering-custom"
---
...
Otherwise:
- Check `templates/steering-custom/` in this skill's directory for available templates (repo override: `{{KIRO_DIR}}/settings/templates/steering-custom/`)
- Read `rules/steering-principles.md` from this skill's directory for steering principles
- **Consult cross-spec learnings before writing (`${CLAUDE_SKILL_DIR}/rules/global-context-loading.md` Rule 6)**: glob-read `{{KIRO_DIR}}/learnings/*.md`. If a pattern already recorded there overlaps the custom steering topic being created, fold it in or cross-reference it — do not author a new, disconnected steering rule that duplicates it.
...
2. **Check if template exists**:
   - Load from `templates/steering-custom/{name}.md` in this skill's directory (repo override: `{{KIRO_DIR}}/settings/templates/steering-custom/{name}.md`) if available
...
Templates available in `templates/steering-custom/` (this skill's directory):
