...
- Paths to spec files (`requirements.md`, design file(s), `tasks.md`) and to `{spec_dir}/learnings.md`, `{spec_dir}/decisions.md`, `{{KIRO_DIR}}/learnings/patterns.md` — **fallback only**, for opening adjacent context not already inlined (patterns.md is cross-spec — open it for generalizable lessons)
...
- **ALL steering verbatim** — every `{{KIRO_DIR}}/steering/*.md` file (the core defaults `product.md`, `tech.md`, `structure.md` AND every custom file present: security, API conventions, domain rules, …) under a "Steering (project-wide rules — binding)" heading. The parent injects the whole set, never the trio alone and never cherry-picked. These are binding conventions, not optional background — your code MUST conform to them.
...
- If your unit touches a convention, contract, or component whose detail was NOT inlined, **actively open** the relevant file under `{{KIRO_DIR}}/steering/`, the design docs, or `{{KIRO_DIR}}/learnings/patterns.md` (for a generalizable cross-spec pattern) rather than guessing — do not silently invent an approach
