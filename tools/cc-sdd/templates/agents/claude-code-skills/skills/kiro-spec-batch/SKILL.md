...
4. For each pending feature in `## Specs (dependency order)`, verify `{{KIRO_DIR}}/specs/<feature>/brief.md` exists (also check `specs/features/<feature>/brief.md`, `specs/bugs/<feature>/brief.md`, and `specs/tech-debt/<feature>/brief.md`)
...
3. Load steering + cross-spec learnings ONCE, before the pipeline below. You are NOT invoked as a
   real skill here, so `${CLAUDE_SKILL_DIR}` will not auto-resolve for you and no single hardcoded
   path is safe — the plugin install and the npx install bundle skills under different roots.

   Resolve bundled paths this way:
   - For each phase SKILL.md you read, treat that file's directory as `${CLAUDE_SKILL_DIR}` for that
     phase.
   - When the SKILL.md says `${CLAUDE_SKILL_DIR}/rules/<file>`, read `<phase-skill-dir>/rules/<file>`.
   - When the SKILL.md says `${CLAUDE_SKILL_DIR}/scripts/<file>`, run `<phase-skill-dir>/scripts/<file>`.
   - Do not reuse a `rules/` or `scripts/` directory from another phase unless it is the sibling of the
     SKILL.md you are executing.

   Use `Glob` for `**/rules/global-context-loading.md` only to find the bundled copy for this initial
   context load. Read it and follow it in full (glob-all `{{KIRO_DIR}}/steering/*.md` and
   `{{KIRO_DIR}}/learnings/*.md`, print the context manifest).
4. Execute the full spec pipeline. For each phase, read the corresponding skill's SKILL.md for complete instructions (templates, rules, review gates) — its own steering/learnings-loading step is already satisfied by step 3 above, do not reload:
...
5. Set all approvals to true in spec.json (auto-approve mode, equivalent of -y flag)
6. Report completion with file list and task count, and confirm in the report that the context manifest from step 3 was printed
```
...
- {{KIRO_DIR}}/specs/*/design.md and {{KIRO_DIR}}/specs/*/*/design.md (primary: contains interfaces, data models, architecture)
- {{KIRO_DIR}}/specs/*/requirements.md and {{KIRO_DIR}}/specs/*/*/requirements.md (for scope and acceptance criteria)
- {{KIRO_DIR}}/specs/*/tasks.md and {{KIRO_DIR}}/specs/*/*/tasks.md (for boundary annotations only -- read _Boundary:_ lines, skip task descriptions)
...
1. Glob `{{KIRO_DIR}}/specs/*/tasks.md` and `{{KIRO_DIR}}/specs/*/*/tasks.md` to verify all specs exist (supports both old flat and new categorized structures)
