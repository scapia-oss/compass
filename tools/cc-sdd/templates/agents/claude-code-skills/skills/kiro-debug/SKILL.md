...
metadata:
  shared-rules: "codebase-grounding.md, global-context-loading.md, learning-promotion.md"
  shared-scripts: "validate-patterns-append-only.py"
...
### 0. Load steering + learning context (do this first — known traps live here)
Follow `${CLAUDE_SKILL_DIR}/rules/global-context-loading.md` in full: glob-all
`{{KIRO_DIR}}/steering/*.md` (a fix must not contradict a project convention) and
`{{KIRO_DIR}}/learnings/*.md` (cross-spec traps — they often already name the failure mode), read
`{spec_dir}/learnings.md` and `{spec_dir}/decisions.md` if present (skip silently otherwise; the spec
dir is the directory of the provided spec files), and print the context manifest.
...
### 7. Capture a generalizable trap (learning)
You read the learning context in step 0; close the loop by recording any **generalizable** trap this
investigation uncovered — a wrong assumption, an environment/version gotcha, or a root-cause class that
would save a future spec from the same dead end. Skip purely spec-specific or one-off causes.

- **When invoked standalone** (`/kiro:debug`, no `impl` parent): if the root cause is generalizable,
  record it **synchronously now** with Write/Edit (do NOT defer to a background subagent — it can be
  dropped when the turn ends): append a LEARNING for phase `implementation` to `{spec_dir}/learnings.md`
  (create from `templates/specs/learnings.md` in this skill's directory — repo override: `{{KIRO_DIR}}/settings/templates/specs/learnings.md` — if missing), then follow
  `rules/learning-promotion.md`: dedupe against `{{KIRO_DIR}}/learnings/*.md`, update
  `{{KIRO_DIR}}/learnings/patterns.md` append-only, include `Source spec` pointing back to this
  learning entry, and run `validate-patterns-append-only.py`.
- **When dispatched as a subagent** (under `/kiro:impl`): do NOT write learnings yourself — surface the
  trap in your Debug Report; the parent `impl` loop records it (its "Record learnings" step fires on
  debug-driven directional changes), avoiding double-recording.
