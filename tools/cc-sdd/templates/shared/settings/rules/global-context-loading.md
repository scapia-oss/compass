...
There are exactly two project-global stores, both siblings under `{{KIRO_DIR}}/`, never inside
`{spec_dir}`:

| Store | Path | Scope | Load rule |
|---|---|---|---|
| Steering | `{{KIRO_DIR}}/steering/*.md` | Project-wide conventions, architecture, domain rules | Glob-all, always |
| Cross-spec learnings | `{{KIRO_DIR}}/learnings/*.md` | Patterns/traps recorded from other specs | Glob-all, always |
...
Load **every** file matching `{{KIRO_DIR}}/steering/*.md` and **every** file matching
`{{KIRO_DIR}}/learnings/*.md`. Do not hardcode filenames (not `product.md, tech.md, structure.md`,
...
real file count in that directory at read time (`count == ls {{KIRO_DIR}}/steering/*.md`, and same
...
| Cross-spec learning | One relevant learning from loaded `{{KIRO_DIR}}/learnings/*.md` with file:line + pattern id/title, or `none found after reading all learning files` | How it shapes this output, or `no change` |
...
`Learning applied: {{KIRO_DIR}}/learnings/<file>.md:<line> - P-N <title> -> <decision/conclusion>`.
...
- Pass an occasional-relevance cross-spec traps file (unbounded size — e.g. the project's `patterns.md`
  under `{{KIRO_DIR}}/learnings/`) as a **fallback path** in the dispatch rather than inlining
  wholesale — tell the subagent to open it for a generalizable pattern relevant to its unit.
- **Exception: `{{KIRO_DIR}}/learnings/style-guide.md` is project-global but gets the FULL-INLINE
...
`{{KIRO_DIR}}/learnings/*.md` first — a cross-spec pattern already recorded there should not be
...
- **Discovery/routing decisions** (`kiro-discovery`) also consult `{{KIRO_DIR}}/learnings/*.md` before
