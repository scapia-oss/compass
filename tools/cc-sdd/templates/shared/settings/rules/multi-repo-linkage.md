...
- **Sanctioned cross-repo write = the pointer ONLY.** cc-sdd normally never writes into another repo.
  ...
  repo's own `{{KIRO_DIR}}/specs/…`, a minimal `spec.json`) into a sibling repo that the *same
...
`{{KIRO_DIR}}/specs/<category>/<YYYY-MM-DD-slug>/` — same category (`features`|`bugs`|`tech-debt`|`chores`),
...
1. When impl edits a file, resolve its git repo root (`git -C <dir> rev-parse --show-toplevel`). If it
   ...
   `dist/`, `build/`, `.git/`, or any generated/dependency tree — never create `{{KIRO_DIR}}/specs/…`
...
`{{KIRO_DIR}}/specs/<category>/<slug>/spec.json` **in the child repo**:
...
parent:
  ...
  spec_path: {{KIRO_DIR}}/specs/<category>/<YYYY-MM-DD-slug>/
...
peers:
  - { repo: <sibling repo>, remote: <url>, spec_path: {{KIRO_DIR}}/specs/<category>/<slug>/ }
