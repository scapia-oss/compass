...
`.kiro/learnings/patterns.md` without breaking existing references.
...
- It is not already covered by an existing global pattern after reading every `.kiro/learnings/*.md`.

Keep one-off corrections in `{spec_dir}/learnings.md`. Use `{spec_dir}/decisions.md` for a chosen
alternative.

## Append-Only Contract
`.kiro/learnings/patterns.md` is append-only.
