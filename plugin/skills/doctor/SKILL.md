...
- **CAN**: repo wiring (`.kiro/` layout, steering present, and any OPTIONAL `.kiro/settings/`
...
- `.kiro/` exists in the project root; `.kiro/steering/*.md` present.
- `.kiro/settings/` is OPTIONAL — it holds user overrides of bundled defaults. Absent is the
  normal, healthy default (skills bundle their own rules/scripts/templates and resolve them from the plugin).
- `.kiro/specs/` structure is sane; `.kiro/learnings/` may be empty or not yet exist — consuming skills glob-all `.kiro/learnings/*.md`, so this is fine either way.
...
- **`settings overrides`** → informational, never a failure. "none — using plugin defaults" is healthy
  (skills are self-contained); "present" just means you keep local overrides under `.kiro/settings/`.
- **`.kiro/ present` = FAIL** → not a kiro repo or wrong directory; run `/kiro:steering` or
  `/kiro:spec-init`.
...
- **`settings overrides up-to-date` = WARN** → a file you override under `.kiro/settings/` differs
...
- **Steering is meaningful, not stubs**: skim `.kiro/steering/*.md` — are product/tech/structure
  actually filled in, or still template placeholders? Stub steering loads but gives the skills nothing.
- **spec.json coherence**: for each spec under `.kiro/specs/`, does `spec.json` parse and is its
...
- **Custom steering is loadable**: if there are steering files beyond product/tech/structure, note them
  so the user knows the consuming skills will load them (they load all of `.kiro/steering/*.md`).
