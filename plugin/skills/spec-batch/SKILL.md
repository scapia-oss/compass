...
4. For each pending feature in `## Specs (dependency order)`, verify `.kiro/specs/<feature>/brief.md` exists (also check `specs/features/<feature>/brief.md`, `specs/bugs/<feature>/brief.md`, and `specs/tech-debt/<feature>/brief.md`)
...
3. Load steering + cross-spec learnings ONCE, before the pipeline below. You are NOT invoked as a
   ...
   context load. Read it and follow it in full (glob-all `.kiro/steering/*.md` and
   `.kiro/learnings/*.md`, print the context manifest).
...
- .kiro/specs/*/design.md and .kiro/specs/*/*/design.md (primary: contains interfaces, data models, architecture)
- .kiro/specs/*/requirements.md and .kiro/specs/*/*/requirements.md (for scope and acceptance criteria)
- .kiro/specs/*/tasks.md and .kiro/specs/*/*/tasks.md (for boundary annotations only -- read _Boundary:_ lines, skip task descriptions)
...
1. Glob `.kiro/specs/*/tasks.md` and `.kiro/specs/*/*/tasks.md` to verify all specs exist (supports both old flat and new categorized structures)
