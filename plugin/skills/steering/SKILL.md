...
- For Bootstrap mode: Read templates from `templates/steering/` in this skill's directory (repo override: `.kiro/settings/templates/steering/`)
...
- **Consult cross-spec learnings before writing (`${CLAUDE_SKILL_DIR}/rules/global-context-loading.md` Rule 6)**: glob-read `.kiro/learnings/*.md`. If a pattern already recorded there overlaps a steering candidate you are about to write, fold it in or cross-reference it — do not author a new, disconnected steering rule that duplicates it.
...
1. Load templates from `templates/steering/` in this skill's directory (repo override: `.kiro/settings/templates/steering/`)
