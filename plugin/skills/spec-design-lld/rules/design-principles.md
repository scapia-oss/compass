...
- **Plain English**: follow `rules/document-style.md`. Use simple words, active voice, and concrete names. Keep the precision; drop the jargon.
- **Define rare terms inline**: if a technical term really is the clearest option, define it in a few words the first time you use it — don't make the reader look it up.
- **Concise**: keep the useful facts. Shorten by using diagrams, tables, and bullets before prose.
