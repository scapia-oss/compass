...
`.kiro/specs/<category>/<YYYY-MM-DD-slug>/` folder here containing a single `spec-link.md`
...
- If `--from` is a readable local path, read `<from>/.kiro/specs/<spec>/spec.json` and the
...
- **Collision guard (BLOCKING, per `rules/multi-repo-linkage.md`):** inspect any existing
  `.kiro/specs/<category>/<slug>/spec.json` here. Absent → create. Present with
...
- Create `.kiro/specs/<category>/<YYYY-MM-DD-slug>/` if absent (create the category dir too).
