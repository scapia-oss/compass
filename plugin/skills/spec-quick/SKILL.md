...
3. **Check for brief**: if a `/kiro:discovery` brief exists at the temporary flat path
   `.kiro/specs/{name}/brief.md` (or an already-categorized brief), read it as the description
...
4. **Path**: `.kiro/specs/<category>/<YYYY-MM-DD>-<slug>/` (slug = kebab-case, 2–4 words; date
   via `date -u +"%Y-%m-%d"`). Use **Glob** to ensure the path is free; append `-2`, `-3`… if taken.
   Record `spec_path` as `<category>/<YYYY-MM-DD>-<slug>`. Ensure `.kiro/learnings/` exists.
   **If a flat discovery brief was found in item 3** (at `.kiro/specs/{name}/brief.md`), create
   the categorized dir and **move** the brief into it (`mv .kiro/specs/{name}/brief.md
   {spec_dir}/brief.md`) along with any spec-scoped discovery scratch in the same flat dir
   (`decisions.md`, `learnings.md`), then remove the flat `.kiro/specs/{name}/` directory once it
   ...
   dated path match** the resolved `.kiro/specs/<category>/<YYYY-MM-DD>-<slug>/` (item 4). If the
...
(`.kiro/steering/*.md`) and cross-spec learnings, read spec-scoped `decisions.md`/`learnings.md`
...
`.kiro/settings/templates/specs/init.json` as the base shape and apply the profile below; keep
...
---------------------------------------------
  ...
  Path:   .kiro/specs/<category>/<date>-<slug>/
...
## Safety & Fallback
- **Templates missing** (`.kiro/settings/templates/specs/…`): report the missing path; MINIMAL
