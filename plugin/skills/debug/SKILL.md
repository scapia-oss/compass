...
`.kiro/steering/*.md` (a fix must not contradict a project convention) and
`.kiro/learnings/*.md` (cross-spec traps — they often already name the failure mode), read
...
- **When invoked standalone** (`/kiro:debug`, no `impl` parent): if the root cause is generalizable,
  ...
  (create from `templates/specs/learnings.md` in this skill's directory — repo override: `.kiro/settings/templates/specs/learnings.md` — if missing), then follow
  `rules/learning-promotion.md`: dedupe against `.kiro/learnings/*.md`, update
  `.kiro/learnings/patterns.md` append-only, include `Source spec` pointing back to this
