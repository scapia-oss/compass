...
- Prefer modifying or extending an existing building block when it already owns the responsibility.
- Create a new building block only when reuse would blur ownership, add hidden coupling, or change
  behavior for existing callers.
- Record the existing construct checked, and why it was reused or rejected.

## 3. Build vs. Adopt

For each major component in the emerging design, ask whether an external solution is a better fit.
