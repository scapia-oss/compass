...
spec_path: <path under .kiro/specs/ | n/a>
...
unit; a real heatmap reads across `.kiro/feedback/*.md` and is built by whatever aggregates them.
...
- **(LEARNING)** — a durable project learning, not a framework change. → route to spec `learnings.md` /
  global `.kiro/learnings/patterns.md` / `decisions.md` (if a choice between alternatives).
...
- **Check prior feedback files.** Glob `.kiro/feedback/*.md` and count how many previous sessions
...
- **Gates**: which fired (review, validate-*, kiro-gate, git-guard, RED witness)? Correct? False blocks or
  misses?
- **Steering & learnings**: was all of `.kiro/steering/` loaded and used? Prior learnings applied?
...
`.kiro/feedback/*.md`. Both are checkable by someone who wasn't there.
