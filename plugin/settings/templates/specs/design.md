...
- **No current-state narrative.** Do not add a "Current State", "As-Is", or "Existing Architecture"
  section and do not walk through how the system works today — grounding evidence lives in `research.md`
...
## Overview 
Write short sentences: what changes, why it matters, and the scope boundary. Keep all useful facts.
...
> Keep this file self-contained for review. Put decisions and contracts here. Put discovery notes in `research.md`.
...
Map the files and responsibilities for this feature. This section feeds task `_Boundary:_` annotations and implementation briefs. Use the right level of detail:
...
> For repeated structures, describe the pattern once. List individual files only when their responsibility is not clear from the path.
...
##### Service Interface
> **Write code in THIS repo's implementation language** — detect it from build manifests (`pom.xml`/`build.gradle` → Java/Kotlin · `pubspec.yaml` → Dart · `go.mod` → Go · `package.json` → TypeScript) and confirm against `tech.md` steering (steering wins on conflict). The block below is **shape-only pseudocode**; render it in the repo's actual language and idioms (a Java `interface`, a Dart `abstract class`, a Go `interface`, …). **Never default to TypeScript.**
